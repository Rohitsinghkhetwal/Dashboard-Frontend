  ...
  const handleCreateUser = async (e) => {
    ...
    try {
      ...
      let result = await fetch(
        "http://localhost:7800/api/v1/users/createUser",
        {
          method: "POST",
          body: formData,
          headers: {
      ...
      if(!result.ok){
        throw new Error("Failed to create ueser !");
      }
      result = await result.json();
      navigate('/dashboard')

      console.log("this is a result here from create user !", result);
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-slate-400 font-bold">
            Fill in the form below to create New user
          </p>
        </div>
